...
class MusicModel:
    ...
    def separate(self, audio_path: str) -> dict:
        ...
        # Run demucs via the current Python interpreter to avoid Windows PATH issues.
        # Using "demucs" directly fails if the Scripts folder is not on PATH,
        # which is common with Microsoft Store Python installations.
        cmd = [sys.executable, "-m", "demucs", "-n", self.model_name,
        ...
        if result.returncode != 0:
            ...
            cmd_wav = [sys.executable, "-m", "demucs", "-n", self.model_name,
